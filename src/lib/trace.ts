...
/** 알파 마스크 캔버스를 SVG path 데이터로. smooth 는 윤곽 다듬기 횟수. */
export function ringsToPath(cv: HTMLCanvasElement, scale: number, tol: number, smooth = 3): string {
  const rings = traceRings(cv);
  const out: string[] = [];
  for (const raw of rings) {
    const r = simplifyClosed(smoothRing(raw, smooth), tol);
